...
$VerifierVersion = "SYSTEM-2026-05-01.5"
...
function Get-KioskRow {
    ...
    $rows = Invoke-SupabaseJson -Uri "$($Supabase.Url)/rest/v1/kiosks?kiosk_id=eq.$encoded&select=kiosk_id,status,last_heartbeat,uptime,ip_address,mac_address,latency_ms" -Method "GET" -Headers $Supabase.Headers
...
if (-not (Invoke-Check "Archivos instalados" {
    ...
    foreach ($path in @($AgentPath, $AgentCachePath, $HealthCheckPath, $ConfigPath)) {
