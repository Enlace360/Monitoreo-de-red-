...
const agentPath = path.join(root, 'usb-kit-2026-05-09', 'Agente_Enlace360_Service.ps1');
assert.strictEqual(
...
includes('/rest/v1/rpc/enlace360_claim_remote_commands', 'agent claims remote commands via RPC');
includes('/rest/v1/rpc/enlace360_complete_remote_command', 'agent completes remote commands via RPC');
includes('status=eq.pending', 'agent pending filtering remains documented for RPC claim');
notIncludes('/rest/v1/remote_commands', 'agent must not access remote_commands table directly');
...
includes('p_status = $execStatus', 'C2 sends command status through RPC');
includes('Function Update-KioskStatus', 'heartbeat function');
includes('Function Get-AgentIntegrity', 'agent integrity inventory');
includes('Confirm-ServiceRunningStable', 'service integrity rechecks transient stopped state');
...
includes('/rest/v1/network_events', 'network_events endpoint');
