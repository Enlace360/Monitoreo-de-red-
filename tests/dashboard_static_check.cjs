...
includes('const EVENT_FILTERS =', 'dashboard defines event impact filters');
includes("const DEFAULT_EVENT_FILTER = 'impact'", 'dashboard defaults last outages to real impact view');
includes('getEventImpact', 'dashboard derives event impact without a Supabase severity column');
includes('formatEventDate', 'dashboard formats event date and time compactly');
includes('eventFilter', 'dashboard stores selected event impact filter');
includes('setEventFilter', 'dashboard can change event impact filter');
includes('visibleEvents', 'dashboard renders filtered outage events');
includes("label: 'Impacto real'", 'dashboard includes real impact filter label');
includes("label: 'Críticos'", 'dashboard includes critical events filter label');
includes("label: 'Auto-Reparado'", 'dashboard includes auto-repaired filter label');
includes("label: 'Todos'", 'dashboard includes all events filter label');
includes("cause.includes('AUTO-REPARADO')", 'dashboard treats auto-repaired micro-cuts as low impact');
includes("impact !== 'low'", 'dashboard real impact filter hides low impact events');
includes('event-impact-badge', 'dashboard shows impact badge per event');
...
assert(css.includes('.event-filters'), 'dashboard styles event filters');
assert(css.includes('.event-filter-btn'), 'dashboard styles event filter buttons');
assert(css.includes('.event-impact-badge'), 'dashboard styles event impact badges');
assert(css.includes('.event-item.impact-low'), 'dashboard styles low impact events');
assert(css.includes('.event-item.impact-medium'), 'dashboard styles medium impact events');
assert(css.includes('.event-item.impact-high'), 'dashboard styles high impact events');
